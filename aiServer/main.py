...
class ImageClassifier:
    ...
    def check_metadata(self, image_bytes):
        try:
            ...
            exif_data = image._getexif()  # EXIF 데이터를 가져옴

            if exif_data:
                return True  # 메타데이터가 있음
            else:
                return False  # 메타데이터가 없음
        except Exception as e:
            logging.error(f"메타데이터 확인 중 오류 발생: {e}")
            return False
...
async def classify(file: UploadFile = File(...)):
    try:
        ...
        # 메타데이터 확인
        has_metadata = classifier.check_metadata(image_bytes)
        metadata_result = "조작이 아닙니다." if has_metadata else "조작입니다."

        # 이미지 분류 (필요에 따라 유지)
        input_tensor = classifier.preprocess_image(image_bytes)
        classify_result = classifier.classify_image(input_tensor)

        return {"metadata_result": metadata_result, "classify_result": classify_result}
